...
public class WindClashBurstEffect : MonoBehaviour
{
    public static void Spawn(Vector2 position, Vector2 direction, Color startColor, Color endColor, float lifetime, float speed, float size, int particleCount, float spreadAngle, int sortingOrder, Material material)
    {
        GameObject burstObject = new GameObject("WindClashBurst");
    ...
    private static void EmitBurst(ParticleSystem particles, Vector2 position, Vector2 direction, Color color, float speed, float size, float lifetime, int particleCount, float spreadAngle)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                position = position,
                velocity = particleDirection * speed * Random.Range(0.35f, 1f),
                startColor = color,
                startSize = size * Random.Range(0.5f, 1.15f),
    ...
    private static Gradient BuildGradient(Color startColor, Color endColor)
    {
        ...
        gradient.SetKeys(
            new[]
            {
                ...
                new GradientColorKey(new Color(0.65f, 1f, 0.75f, 1f), 0.35f),
