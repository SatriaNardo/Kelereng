...
public class WindTrailEffect : MonoBehaviour
{
    ...
    public void Configure(Color startColor, Color endColor, float particlesPerSecond, float lifetime, float startSize, float speed, int sortingOrder, Material material)
    {
        rb = GetComponent<Rigidbody2D>();

        if (trailParticles == null)
        {
            GameObject particleObject = new GameObject("WindMoveTrail");
    ...
    private void Update()
    {
        ...
        if (shouldEmit)
        {
            ...
            trailParticles.transform.position = (Vector2)transform.position - velocity * 0.22f;
    ...
    private Gradient BuildGradient(Color startColor, Color endColor)
    {
        ...
        gradient.SetKeys(
            new[]
            {
                ...
                new GradientColorKey(new Color(0.75f, 1f, 0.8f, 1f), 0.45f),
